...
public class forkJoinTest extends RecursiveTask<Boolean> {
    ...
    protected Boolean compute() {
        ...
        System.out.println(
                Thread.currentThread().getName() + " says : After splliting the arry length is :: " + arr.length +
                " Midpoint is " + mid);
        if (arr[mid] == searchableElement) {
            ...
            int[] left = Arrays.copyOfRange(arr,
                                            0,
                                            mid);
            forkJoinTest forkTask = new forkJoinTest(left,
                                                     searchableElement);
            forkTask.fork();
            return forkTask.join();
        } else if (searchableElement > arr[mid]) {
            System.out.println(Thread.currentThread().getName() + " says :: Doing Right-search");
            int[] right = Arrays.copyOfRange(arr,
                                             mid,
                                             arr.length);
            forkJoinTest forkTask = new forkJoinTest(right,
                                                     searchableElement);
            forkTask.fork();
            return forkTask.join();
        }
